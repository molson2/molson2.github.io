...
Four of the five curves are doing the same thing: pass the top of the spectrum, suppress the bottom, and differ only in *where* the knee sits and *how abruptly* it turns. PCR and Ridge are drawn with the same crossover at $\sigma^2 = 0.1$, which makes the contrast the only difference between them a step versus a smooth ramp. The two GD curves show early stopping sliding along that same family as $t$ grows. While the correspondence with Ridge and GD isn't exact, there is a very close resemblance when $t=1/(\eta \lambda)$.
